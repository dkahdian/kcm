...
% Generated: 2026-02-12T23:14:25.240Z
% 
...
\end{claim}
\begin{claimdescription}
Corollary 2 of \citet{Kaleyski_2016} shows that for some boolean formula $\psi_i$, a $MODS$ representation of $\psi_i$ is $\Theta(2^{2i})$. Proposition 6 of \citet{Kaleyski_2016} shows that the $PI$ representation of the same $\psi_i$ is $\Omega(2^{\frac{(i-1)^2-1}{4}})$, which is not polynomial in $2^{2i}$. Thus, no polynomial transformation exists.

Chapter 4 of \citet{Kaleyski_2016} demonstrates that a similar analysis is insufficient to produce an exponential separation of $PI$ and $MODS$, so whether such a separation exists is still an open problem.
\end{claimdescription}
