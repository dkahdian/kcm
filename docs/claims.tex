...
% Generated: 2026-02-18T00:54:53.255Z
% 
...
\end{claim}
\begin{claimdescription}
As formalized by \citet{Darwiche_2002} and originally introduced in Darwiche (2001), $d$-$DNNF$ is a subset of $DNNF$ satisfying the determinism property, making a polynomial transformation trivial.
\end{claimdescription}
...
\end{claim}
\begin{claimdescription}
Because counting the models of a $d$-$DNNF$ is in P and counting the models of a $DNF$ is \#P-complete, $DNF$ cannot be polynomially compiled into $d$-$DNNF$ unconditionally. Such a transformation implies P = \#P, collapsing the polynomial hierarchy \citep{Darwiche_2002}.
\end{claimdescription}
...
\end{claim}
\begin{claimdescription}
While \citet{Darwiche_2002} cites specific examples like the negation of the pairwise DNF $\Pi_n$, the exponential blowup is fundamentally a structural translation issue: distributing a $DNF$ (of which $IP$ is a subset) into a $CNF$ inherently risks an exponential explosion of clauses.
\end{claimdescription}
...
\end{claim}
\begin{claimdescription}
While \citet{Bova_2016} generalizes this separation using communication complexity on the $JS_n$ function (Proposition 11 showing $PI$ size $O(n^2)$ and Proposition 12 showing $DNNF$ size $2^{\Omega(n^2)}$), the foundational unconditional strongly exponential separation of $CNF$ (and by extension $PI$) from $DNNF$ was originally established in \citet{Bova_2014}.
\end{claimdescription}
...
\end{claim}
\begin{claimdescription}
As established by \citet{Darwiche_2011}, which introduced SDDs as a new central canonical representation, every $OBDD$ is a right-linear $SDD$. Thus, $SDD$ strictly generalizes $OBDD$.
\end{claimdescription}
