...
\begin{itemize}
\item{If there is a polynomial-time translator from $L_1$ to $L_2$ and $L_2$ supports $q$, then $L_1$ supports $q$ (translate, then query).}
\item{Corollary: if $L_1$ does not support $q$, but has a polynomial-time translator to $L_2$, then $L_2$ does not support $q$ either.}
\item{Corollary: if $L_1$ supports $q$, but $L_2$ does not support $q$, then no polynomial-time translator exists from $L_2$ to $L_1$.}
\end{itemize}

Polynomial-size representability and polynomial-time translatability are distinct: a translator gives a polynomial-size target representation, but a size upper bound alone does not supply a translator. Thus a query difference can rule out a polynomial-time translator, but is not a succinctness lower bound.
...
\textbf{Query-by-compile validation.} We consider each pair of languages $L_i, L_j \in \textbf{L}$ so that a path of known polynomial-time translators exists from $L_i$ to $L_j$. For such pairs, we ensure that every query $q$ supported by $L_j$ is not unsupported by $L_i$. This requires a similar DFS algorithm taking $O((\#\textbf{L})^2(\#Q)$ time.
...
\paragraph{Via translation (contrapositive of query-by-compiling).}
If $L_1.q = \texttt{no-poly}$ and there is a polynomial-time translator from $L_1$ to $L_2$, then $L_2.q = \texttt{no-poly}$. (If $L_2.q = \texttt{poly}$, $L_1$ could translate to $L_2$ and query there---contradiction.) Checking this for each pair of languages and for each query takes $O((\#\textbf{L})^2)(\#Q))$ per iteration.
...
\subsection{Phase 6: Translation Downgrades via Query Differences}

For each pair of languages $L_1, L_2$ and each query $q$, if $q$ is unsupported by $L_1$ but supported by $L_2$, we may record that no polynomial-time translator exists from $L_1$ to $L_2$ (for otherwise we could query $L_1$ by translating to $L_2$). This does not establish a succinctness lower bound. This takes at most $O((\#\textbf{L})^2(\#Q))$ time per iteration.

Since Phase~6 may add translatability facts that affect the earlier query phases, the entire propagator (Phases~1--6) is wrapped in a global fixed-point loop, although usually this requires no more than 2 iterations to converge.
