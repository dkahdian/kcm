...
Succinctness and translatability are tracked separately. A polynomial
translatability fact asserts a polynomial-time compiler and therefore implies
the corresponding polynomial succinctness fact. The converse is not used.
Query-by-compiling, including its contrapositives, uses only translatability:
a query difference can rule out a polynomial-time compiler, but cannot by
itself rule out polynomial-size target representations.
