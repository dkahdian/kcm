...
\newcommand{\nOBDD}{\mathrm{nOBDD}}
\newcommand{\uOBDD}{\mathrm{uOBDD}}
\newcommand{\SDNNF}{\mathrm{SDNNF}}
\newcommand{\dSDNNF}{\mathrm{d}\mbox{-}\mathrm{SDNNF}}
\newcommand{\decSDNNF}{\mathrm{dec}\mbox{-}\mathrm{SDNNF}}

\title{A Common-Order Quasipolynomial Simulation from SDNNF to nOBDD}
\author{}
\date{\today}
...
DNNFs by nondeterministic OBDDs.  The usual simulation orients each vtree split
...
nOBDDs using one common variable order, while preserving a quasipolynomial size
bound.
...
Let $D$ be a normalized $\SDNNF$ respecting $T$.  Literals, if present, are
...
heavy-side input is a \emph{heavy edge}.  Edges out of decision nodes and OR
nodes are \emph{neutral edges}.
...
\begin{enumerate}[label=(\roman*),noitemsep]
  ...
  \item If $u$ is an OR gate, then $(u,s)$ is an OR node.
...
decision nodes and OR nodes.  Thus the result is a nondeterministic BDD. However,
we can show that all paths respect the same order $\pi_X$, hence the result is
an $\nOBDD_{\pi_X}$.

\section{Why the Order is Common}

\begin{lemma}[Order lemma]
Let $D$ be a normalized $\SDNNF$ respecting $T$.  The nOBDD produced by the
...
Decision gates and OR gates do not change the vtree region in which the
computation is located; they only move within the current region.  Hence along
every accepting or rejecting computation path, variables are queried in
increasing $\pi_X$ order, with variables absent from $D$ simply skipped.
\end{proof}

\begin{corollary}[Same order for all circuits respecting the same tree]
Let $D_1,\ldots,D_k$ be normalized $\SDNNF$s respecting the
same finite vtree $T$ over variables $X$.
Then the construction compiles every $D_i$ into an $\nOBDD$ respecting the same
variable order $\pi_X$.
\end{corollary}
...
For every normalized $\SDNNF$ $D$ respecting $T$,
...
Let $D_1,\ldots,D_k$ be normalized $\SDNNF$ circuits respecting the same finite
vtree $T$ over variables $X$.
For each $i$, the construction above outputs an $\nOBDD$ for $D_i$ respecting the
...
$L\leq \lceil \log_2 |X| \rceil+1$.  After contracting no-op nodes, the output
has only decision nodes, OR nodes, and sinks, so it is an nOBDD.
\end{proof}

\begin{corollary}[Deterministic inputs give unambiguous outputs]
If each $D_i$ is a normalized $\dSDNNF$ respecting $T$, then the construction
outputs $\uOBDD$s respecting the same order $\pi_X$ and satisfying the same
quasipolynomial size bound.
\end{corollary}

\begin{proof}
The common-order and size claims are exactly those of the theorem.  It remains
only to check unambiguity.  The Bollig--Buttkus argument shows that, for this
simulation, accepting paths of the output correspond to $1$-certificates of the
input.  In a deterministic DNNF, every satisfying assignment has exactly one
$1$-certificate.  Hence every satisfying assignment has at most one accepting
path in the output nOBDD.
\end{proof}

\begin{corollary}[Decision inputs give deterministic outputs]
If each $D_i$ is a normalized $\decSDNNF$ respecting $T$, then the construction
outputs $\OBDD$s respecting the same order $\pi_X$ and satisfying the same
quasipolynomial size bound.
\end{corollary}

\begin{proof}
A decision structured DNNF can first be put in the equivalent form where each
decision OR is represented as a decision node labeled by its tested variable.
Then the simulation introduces no nondeterministic OR nodes.  After contracting
no-op nodes, the output has only deterministic decision nodes and sinks.
\end{proof}

\begin{corollary}[Two size-$N$ circuits]
If $D_1$ and $D_2$ both have size at most $N$, then
Since $M_i, |X|\leq N$, both circuits compile to $\nOBDD$s over one common order of
size
\[
  O(NN^{\log(N) + 1}) = N^{O(\log N)}
\].
...
shared finite vtree.  For any fixed finite family of SDNNF circuits respecting
that vtree, this gives one common nOBDD order and keeps the simulation
quasipolynomial in the total input size.  Deterministic inputs give unambiguous
nOBDDs, and decision inputs give OBDDs.
