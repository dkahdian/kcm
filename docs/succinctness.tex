...
% Generated: 2026-04-08T00:35:09.453Z
% 
...
% =============================
\begin{claim}
\langref{d-DNNF} has unknown polynomial-time (but has quasi-polynomial-time) compilation to \langref{uFBDD} \citet{Amarilli_2018,Beame_2013}
\end{claim}
...
\begin{claim}
\langref{dec-SDNNF} is not polynomial-time compilable to \langref{nFBDD} \citet{Amarilli_2018}
\end{claim}
\begin{claimdescription}
...
\begin{claim}
\langref{DNF} is polynomial-time compilable to \langref{nOBDD} \citet{Amarilli_2018,Wegener_2000}
\end{claim}
\begin{claimdescription}
Fix any variable order $<$. Each term of a \langref{DNF} can be represented as an \langref{OBDD\_<} of linear size (a chain of decision nodes for each literal in the term). The nondeterministic disjunction of these \langref{OBDD\_<}s yields an \langref{nOBDD} of size $O(|\phi|)$, since the variable order is shared across all branches \citet{Amarilli_2018,Wegener_2000}.
\end{claimdescription}
...
\begin{claim}
\langref{nOBDD} is not quasi-polynomial-time compilable to \langref{uOBDD} \citet{Amarilli_2018}
\end{claim}
\begin{claimdescription}
...
% separators=HWB
\begin{claim}
\langref{uOBDD} is not quasi-polynomial-time compilable to \langref{OBDD} \citet{Amarilli_2018}
\end{claim}
\begin{claimdescription}
...
\begin{claim}
\langref{DNF} is not polynomial-time compilable to \langref{d-DNNF} unless the polynomial hierarchy collapses \citet{Darwiche_2002}
\end{claim}
\begin{claimdescription}
...
\begin{claim}
\langref{MODS} is polynomial-time compilable to \langref{OBDD\_<} \citet{Darwiche_2002}
\end{claim}
\begin{claimdescription}
A Shannon tree for any \langref{MODS} formula following order $<$ can be generated in polynomial time. This tree can be reduced into a corresponding \langref{OBDD\_<} in polynomial time \citet{Darwiche_2002}.
\end{claimdescription}
...
\begin{claim}
\langref{OBDD} is not quasi-polynomial-time compilable to \langref{OBDD\_<} \citet{Darwiche_2002}
\end{claim}
\begin{claimdescription}
The function $\bigwedge_{i=1}^n(x_i\Leftrightarrow y_i)$ has an \langref{OBDD\_<} representation of size polynomial in $n$ whenever $<$ satisfies $x_1<y_1<...<x_n<y_n$ but exponential in $n$ if $<$ satisfies $x_1<x_2<...<x_n<y_1<y_2<...<y_n$
\end{claimdescription}

% separators=Parity
\begin{claim}
\langref{OBDD\_<} is not quasi-polynomial-time compilable to \langref{CNF} \citet{Darwiche_2002}
\end{claim}
\begin{claimdescription}
The parity function $PARITY_n=\bigoplus_{i=1}^n x_i$ has linear size \langref{OBDD\_<} representations but only exponential size \langref{CNF} representations.
\end{claimdescription}

% separators=Parity
\begin{claim}
\langref{OBDD\_<} is not quasi-polynomial-time compilable to \langref{DNF} \citet{Darwiche_2002}
\end{claim}
\begin{claimdescription}
The parity function $PARITY_n=\bigoplus_{i=1}^n x_i$ has linear size \langref{OBDD\_<} representations but only exponential size \langref{DNF} representations.
\end{claimdescription}

\begin{claim}
\langref{OBDD\_<} is polynomial-time compilable to \langref{OBDD} \citet{Darwiche_2002}
\end{claim}
\begin{claimdescription}
As defined by \citet{Darwiche_2002}, \langref{OBDD\_<} is a subset of \langref{OBDD}. Therefore, a polynomial compilation is trivial.
\end{claimdescription}
...
% =============================
\begin{claim}
\langref{SDD} is not quasi-polynomial-time compilable to \langref{FBDD} \citet{Bollig_2018}
\end{claim}
\begin{claimdescription}
...
\begin{claim}
\langref{SDD\_T} is polynomial-time compilable to \langref{SDD} \citet{Darwiche_2011}
\end{claim}
\begin{claimdescription}
\langref{SDD} is the union of the fixed-vtree languages \langref{SDD\_T}. Hence any \langref{SDD\_T} formula is already an \langref{SDD} formula, so the compilation is trivial and polynomial-time.
\end{claimdescription}

\begin{claim}
\langref{cSDD\_T} is polynomial-time compilable to \langref{cSDD} \citet{Darwiche_2011}
\end{claim}
\begin{claimdescription}
\langref{cSDD} is the union of the fixed-vtree languages \langref{cSDD\_T}. Hence any \langref{cSDD\_T} formula is already a \langref{cSDD} formula, so the compilation is trivial and polynomial-time.
\end{claimdescription}


% =============================
\section{K. Pipatsrisawat and A. Darwiche, "A New D-DNNF-Based Bound for #SAT", 2008.}
% Reference ID: Pipatsrisawat_2008
% =============================
\begin{claim}
\langref{SDNNF\_T} is polynomial-time compilable to \langref{SDNNF} \citet{Pipatsrisawat_2008}
\end{claim}
\begin{claimdescription}
\langref{SDNNF} is the union of the fixed-vtree families \langref{SDNNF\_T}. Hence any \langref{SDNNF\_T} formula is already an \langref{SDNNF} formula, so the compilation is trivial and polynomial-time.
\end{claimdescription}

\begin{claim}
\langref{d-SDNNF\_T} is polynomial-time compilable to \langref{d-SDNNF} \citet{Pipatsrisawat_2008}
\end{claim}
\begin{claimdescription}
\langref{d-SDNNF} is the union of the fixed-vtree languages \langref{d-SDNNF\_T}. Hence any \langref{d-SDNNF\_T} formula is already a \langref{d-SDNNF} formula, so the compilation is trivial and polynomial-time.
\end{claimdescription}


% =============================
\section{U. Oztok and A. Darwiche, "A Top-Down Compiler for Sentential Decision Diagrams", 2014.}
% Reference ID: Oztok_2014
% =============================
\begin{claim}
\langref{dec-SDNNF\_<} is polynomial-time compilable to \langref{dec-SDNNF} \citet{Oztok_2014}
\end{claim}
\begin{claimdescription}
\langref{dec-SDNNF} is the union of fixed-vtree \langref{dec-SDNNF\_<} languages. Hence any fixed-family formula is already a \langref{dec-SDNNF} formula, so the compilation is trivial and polynomial-time.
\end{claimdescription}
