...
% Generated: 2026-04-23T01:13:26.557Z
% 
...
\end{claim}
\begin{claimdescription}
For any fixed vtree $T$, Theorem 2 of \citet{Bollig_2018} gives a quasipolynomial simulation from structured \langref{DNNF} to \langref{nOBDD} that follows an induced variable order $<$. Proposition 2 of \citet{Bollig_2018} gives unambiguity for deterministic inputs, yielding a quasipolynomial compilation from \langfam{d-SDNNF}{T} to \langfam{uOBDD}{<}.
\end{claimdescription}
...
\end{claim}
\begin{claimdescription}
Fix any variable order $<$. Each term of a \langref{DNF} can be represented as an \langfam{OBDD}{<} of linear size (a chain of decision nodes for each literal in the term). The nondeterministic disjunction of these \langfam{OBDD}{<}s yields an \langref{nOBDD} of size $O(|\phi|)$, since the variable order is shared across all branches \citet{Amarilli_2018,Wegener_2000}.
\end{claimdescription}
...
\begin{claim}
\langfam{SDNNF}{T} has unknown polynomial-time (but has quasi-polynomial-time) compilation to \langfam{nOBDD}{<} \citet{Bollig_2018,Beame_2015}
\end{claim}
\begin{claimdescription}
For any fixed vtree $T$, Theorem 2 of \citet{Bollig_2018} gives a quasipolynomial simulation from \langfam{SDNNF}{T} to a nondeterministic OBDD following an induced variable order $<$. Hence \langfam{SDNNF}{T} has quasipolynomial compilation to \langfam{nOBDD}{<}.
\end{claimdescription}
