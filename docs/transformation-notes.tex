...
\subsection{Quasi-polynomial variants}
The same argument works with ``not in quasi-polynomial time'' in place of ``not in polynomial time.''
...
\begin{itemize}[noitemsep]
  \item \langref{d-DNNF}: the natural $\lor$C argument should give a conditional ``not in polynomial time; quasi-polynomial open'' result, but this is already present through propagation.
