...
As a result, because \compilespoly{\langref{OBDD}}{\thislang} and \compilespoly{\thislang}{\langref{DNNF}}, \thislang cannot support bounded conjunction in polynomial time.
\end{description}
\end{batchclaim}
...
As a result, because \compilespoly{\langref{OBDD}}{\thislang} and \compilespoly{\thislang}{\langref{SDNNF}}, \thislang cannot support bounded disjunction in polynomial time.
\end{description}
\end{batchclaim}
...
As a result, because \compilespoly{\langref{OBDD}}{\thislang} and \compilespoly{\thislang}{\langref{d-DNNF}}, \thislang cannot support bounded disjunction in polynomial time.
...
\begin{batchclaim}
\selector{\isin{\langref{d-DNNF}, \langref{dec-DNNF}, \langref{uFBDD}, \langfam{d-SDNNF}{T}, \langfam{dec-SDNNF}{T}, \langfam{uOBDD}{<}}}
\operation{\SFO}
\status{\nopolyunknownquasi}
\begin{description}
If \thislang supported SFO in polynomial time, then given two \thislang circuits $f$ and $g$ we could form the decision $(x \land f) \lor (\neg x \land g)$ with only constant overhead and forget $x$, yielding $f \lor g$. This would give polynomial-time Bounded Disjunction for \thislang, contradicting the lower bound of \citet{Vinall-Smeeth_2024}. \citep{TCZ_Forthcoming}
\end{description}
\end{batchclaim}
...
\begin{batchclaim}
\selector{\compilespoly{\langref{CNF}}{\thislang}}
\operation{\FO}
\status{\nopolyunknownquasi}
\assuming{P \neq NP}
\begin{description}
If \thislang supported FO, then the consistency of any \langref{CNF} representation $\Sigma$ could be tested in polynomial time: represent $\Sigma$ in \thislang, compute $\exists\text{Vars}(\Sigma).\Sigma$, and check the resulting constant representation. This would decide \langref{CNF} consistency in polynomial time, implying $\mathrm{P = NP}$ \citep{Darwiche_2002}.
\end{description}
\end{batchclaim}

\begin{batchclaim}
\selector{\isin{\langref{OBDD}, \langref{SDD}, \langref{cSDD}, \langref{dec-SDNNF}, \langref{TDD}, \langfam{OBDD}{<}, \langfam{SDD}{T}, \langfam{cSDD}{T}, \langfam{TDD}{T}}}
\operation{\FO}
\status{\nopolyunknownquasi}
\begin{description}
For every \langref{DNF} representation $\Gamma$, \citet{TCZ_Forthcoming} constructs a polynomial-size \langfam{OBDD}{<}
representation $B_\Gamma$ over fresh selector variables $V$ such that $\Gamma \equiv \exists V.B_\Gamma$. Moreover, \compilespoly{\langfam{OBDD}{<}}{\thislang}, so $B_\Gamma$ has an equivalent polynomial-size \thislang representation. If \thislang supported FO in polynomial time, forgetting $V$ from this representation would yield a polynomial-size \thislang representation of $\Gamma$. This contradicts \nocompilespoly{\langref{DNF}}{\thislang}.
\end{description}
\end{batchclaim}
...
\status{\nopolyunknownquasi}
\begin{description}
\citet[Theorem 6.3.2]{Wegener_2000} shows that there are Boolean functions $f,g:\{0,1\}^n\to\{0,1\}$, each computable by \langref{OBDD}{s} of size $O(n)$, such that any \langref{FBDD} computing $f\lor g$ has size $2^{\Omega(\sqrt{n})}$.
\end{description}
\end{transformationclaim}
...
\status{\poly}
\begin{description}
Every \langref{SDD} representation $f$ respects some vtree $T$. Conditioning $f$ on $x$ and on $\neg x$ produces two \langref{SDD} representations respecting the same pruned vtree $T \setminus \{x\}$. Applying bounded disjunction to these compatible representations yields $f|_{x=0} \lor f|_{x=1} \equiv \exists x.f$ in polynomial time \citep{Darwiche_2011}.
\end{description}
\end{transformationclaim}
